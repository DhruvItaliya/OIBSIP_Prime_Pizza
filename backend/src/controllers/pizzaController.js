...
export const fetchToppings = async (req, res) => {
    try {
        const toppings = await pizzaService.fetchToppings();
        res.status(200).json({ success: true, toppings });
    }
    catch (error) {
        console.log("Error in fetch bases");
        if (error instanceof Error) {
...
export const fetchRecommended = async (req, res) => {
    try {
        const recommended = await pizzaService.fetchRecommended();
...
export const fetchPizzaMania = async (req, res) => {
    try {
        const mania = await pizzaService.fetchPizzaMania();
        res.status(200).json({ success: true, mania });
    }
    catch (error) {
        console.log("Error in new launches");
        if (error instanceof Error) {
